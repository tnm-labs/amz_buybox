...
def compute_listing_type_breakdown(df):
    """
    Breakdown by Listing_Type (Fitment/Installation, Only Delivery, Latched On).
    Only Delivery and Fitment/Installation listings are ones we typically created/own
    outright, so Buy Box win rate should ideally be 100%. Latched On listings piggyback
    on an ASIN that may not be ours originally, so real competition there is expected
    and not flagged. Uses row-level data (not deduplicated across types) since an ASIN
    tracked under two types is a legitimate separate entry for each type.
    """
    ok_df = df[df["status"] == "ok"]
    rows = []
    for name, g in ok_df.groupby("Listing_Type"):
        won = (g["is_buy_box_winner"] == True).sum()
        total = len(g)
        win_pct = round(100 * won / total, 1) if total else 0.0
        should_be_100 = name in ("Only Delivery", "Fitment/Installation")
        flag = should_be_100 and win_pct < 100.0
        rows.append({"name": name, "total": total, "won": int(won), "win_pct": win_pct, "flag": flag})
    order = {"Fitment/Installation": 0, "Only Delivery": 1, "Latched On": 2}
...
def render_tab_content(tab_id, metrics, brand_bd, category_bd, listing_type_bd, priority_rows, priority_title):
    ...
    listing_type_rows = ""
    for lt in listing_type_bd:
        row_style = 'style="background: #fdeaea;"' if lt["flag"] else ""
        flag_note = ' <strong style="color:#c0392b;">- check for hijacker/reseller</strong>' if lt["flag"] else ""
        listing_type_rows += (
            f"<tr {row_style}><td>{lt['name']}</td><td>{lt['total']}</td><td>{lt['won']}</td>"
            f"<td>{lt['win_pct']}%{flag_note}</td></tr>"
        )
  ...
  <table>
    <tr><th>Listing type</th><th>Total</th><th>Won</th><th>Win %</th></tr>
    {listing_type_rows}
  </table>
  <p class="hint">Only Delivery and Fitment/Installation listings are ones we typically
    created and own outright, so their win rate should ideally be 100%. Rows highlighted
    red have dropped below that - worth checking for a hijacker or unauthorized reseller
    on that listing. Latched On listings piggyback on an ASIN we may not have created, so
    real competition there is expected and not flagged.</p>
"""


def render_dashboard(top50_metrics, top50_brand_bd, top50_cat_bd, top50_lt_bd, top50_drilldowns, top50_priority_rows,
                      all_metrics, all_brand_bd, all_cat_bd, all_lt_bd, all_drilldowns, all_priority_rows,
                      fitment_excluded_count=0):
    generated_at = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d %H:%M %Z")

    top50_content = render_tab_content("top50", top50_metrics, top50_brand_bd, top50_cat_bd, top50_lt_bd,
                                        top50_priority_rows, "Top 50 priority listings")
    all_content = render_tab_content("all", all_metrics, all_brand_bd, all_cat_bd, all_lt_bd,
...
def main():
    ...
    full_df = df.copy()  # all listing types, all rows - used only for the listing-type breakdown
    ...
    # doesn't meaningfully apply - exclude them from all other Buy Box metrics.
    fitment_excluded = df[df["Listing_Type"] == "Fitment/Installation"].copy()
    df = df[df["Listing_Type"] != "Fitment/Installation"].copy()

    # A single ASIN can appear under multiple SKUs (e.g. one "Only Delivery" SKU and
    # one "Latched On" SKU pointing to the same live listing). Buy Box status is a
    # property of the ASIN, not the SKU, so counting/displaying it more than once
    # double-counts the same result. Keep one row per ASIN.
    before_dedup = len(df)
    df = df.drop_duplicates(subset="ASIN", keep="first").copy()
    print(f"Deduplicated {before_dedup - len(df)} repeat-ASIN rows (kept one row per ASIN "
          f"for Buy Box metrics; {len(df)} unique ASINs remain).")
    ...
    # Listing-type breakdown uses the full pre-exclusion dataset (includes Fitment/Installation
    # rows and un-deduplicated repeat-ASIN rows), since the whole point here is to see each
    # listing type's own win rate, including the ones normally excluded/deduped elsewhere.
    top50_full = full_df[full_df["Is_Top_50"] == True].copy()
    top50_lt_bd = compute_listing_type_breakdown(top50_full)
    all_lt_bd = compute_listing_type_breakdown(full_df)
    ...
    html = render_dashboard(top50_metrics, top50_brand_bd, top50_cat_bd, top50_lt_bd, top50_drilldowns, top50_priority_rows,
                             all_metrics, all_brand_bd, all_cat_bd, all_lt_bd, all_drilldowns, all_priority_rows,
