...
MASTER_FILE = "data/Marketplace Catalogue Master.xlsx"  # used to link each ASIN's Flipkart counterpart + market Top 500
...
def process_raw_listings(raw):
    ...
    # Amazon's Merchant Listings report never populates `quantity` for FBA-fulfilled
    # offers (fulfillment-channel == AMAZON_IN) - Amazon manages that inventory, not
    # the seller's own listing feed, so a blank value there is NOT a "0 stock" signal.
    # Only drop a row for being out of stock when it's merchant-fulfilled and actually
    # reports 0; keep any FBA row that's still an active marketplace listing.
    is_fba = raw["fulfillment-channel"] == "AMAZON_IN"
    is_live_marketplace = raw["item-is-marketplace"] == "y"
    raw = raw[(raw["quantity"].fillna(0) > 0) | (is_fba & is_live_marketplace)].copy()
...
# ---- Flipkart link matching + market Top 500 opportunity data ----------

def strip_amazon_suffix(sku):
    """Strip Amazon listing-type suffixes off an SKU to get the base TNM SKU code
    (same logic as the catalogue dashboard, kept in sync so both scripts match
    the same base SKU the same way)."""
    s = str(sku)
    return re.sub(r'(ODLONSO[24]|ODLON|LONSO[24]|LON|OD|WF)$', '', s)


def build_flipkart_lookup():
    """Maps base TNM SKU code + listing type -> Flipkart product URL, using the
    FK OD / FK WF / FK Latched On FSN columns in the master catalogue file.
    Returns an empty dict (Flipkart links simply won't show) if the master
    file isn't present yet."""
    if not os.path.isfile(MASTER_FILE):
        print(f"NOTE: {MASTER_FILE} not found - Flipkart links will be blank until it's uploaded.")
        return {}
    try:
        mapping = pd.read_excel(MASTER_FILE, sheet_name="SKU Mapping")
    except Exception as e:
        print(f"NOTE: Could not read SKU Mapping sheet for Flipkart links: {e}")
        return {}

    lookup = {}
    for col, listing_type in [("FK OD", "Only Delivery"), ("FK WF", "Fitment/Installation"),
                                ("FK Latched On", "Latched On")]:
        if col not in mapping.columns:
            continue
        sub = mapping[["TNM SKU Code", col]].dropna()
        for _, r in sub.iterrows():
            lookup[(str(r["TNM SKU Code"]), listing_type)] = (
                f"https://www.flipkart.com/product/p/itme?pid={r[col]}"
            )
    return lookup


def load_amazon_top500_df():
    """Reads the 'Amazon Top 500' sheet from the master catalogue file (market-wide
    best-sellers, including competitor products). Returns None if the file/sheet
    isn't available yet, so callers can degrade gracefully."""
    if not os.path.isfile(MASTER_FILE):
        return None
    try:
        amz_top500 = pd.read_excel(MASTER_FILE, sheet_name="Amazon Top 500")
    except Exception as e:
        print(f"NOTE: Could not read 'Amazon Top 500' sheet from {MASTER_FILE}: {e}")
        return None
    return amz_top500.drop_duplicates(subset="asin").copy()


def build_amazon_top500_tab(df, top500_win_df):
    """Two views on the market's Top 500 best-sellers: (1) presence - do we have
    ANY live listing on each one, regardless of competition, and (2) for the
    ones we ARE live on, what's our Buy Box win rate."""
    amz_top500_unique = load_amazon_top500_df()
    if amz_top500_unique is None:
        return "<p class='note'>Upload the master catalogue file (with an 'Amazon Top 500' sheet) to populate this tab.</p>"

    tnm_live_asins = set(df["ASIN"])
    amz_top500_unique["tnm_present"] = amz_top500_unique["asin"].isin(tnm_live_asins)
    present = int(amz_top500_unique["tnm_present"].sum())
    total = len(amz_top500_unique)
    pct = round(100 * present / total, 1) if total else 0
    color = "#c0392b" if pct < 30 else ("#e67e22" if pct < 60 else "#2F5496")

    win_m = compute_metrics(top500_win_df)

    card_html = f"""<div class="card">
        <div class="card-value" style="color:{color};">{present}/{total} ({pct}%)</div>
        <div class="card-label">Amazon Top 500 - TNM presence</div></div>
    <div class="card">
        <div class="card-value" style="color:#2F5496;">{win_m['buy_box_win_pct']}%</div>
        <div class="card-label">Amazon Top 500 - Buy Box win rate</div>
        <div class="card-sub">{win_m['total_tracked']} of our own listings tracked - {win_m['buy_box_won']} won / {win_m['buy_box_lost']} lost</div></div>"""

    missing = amz_top500_unique[~amz_top500_unique["tnm_present"]].sort_values("Rank")
    missing_rows = "".join(
    ...
    return f"""
  <p class="hint" style="font-size:12px; color:#999; margin-bottom:20px;">
    Best-selling ASINs across the whole Amazon category market (includes competitor products).
    "Presence" measures whether TNM has ANY live listing on the market's actual top sellers.
    "Buy Box win rate" is scoped to only the ones we ARE live on - separate metric.</p>
  <div class="cards">{card_html}</div>
  <h2>Missing from Amazon Top 500 ({len(missing)})</h2>
  <table><tr><th style='{th()}'>Rank</th><th style='{th()}'>ASIN</th><th style='{th()}'>Brand</th><th style='{th()}'>Title</th></tr>
    {missing_rows}
  </table>
"""
...
def build_drilldowns(df):
    ...
    def rows(sub_df):
        # NOTE: buy_box_price equals Your_Price whenever we hold the Buy Box (that's
        # what "holding it" means) - it is NOT a competitor's price. The number that
        # actually says how much room we have to raise price while still winning is
        # `headroom`, so it has to travel through to the drilldown table separately
        # from gap_to_buybox (which is only meaningful on rows where we're losing).
        cols = ["ASIN", "Title", "Brand", "Category", "Your_Price", "buy_box_price",
                "gap_to_buybox", "headroom", "num_offers", "Product_URL", "Flipkart_URL"]
...
def render_product_tab(tab_id, df, priority_title, flag_if_under_100=False):
    ...
    for _, r in product_rows_df.iterrows():
        ...
        fk_url = r.get("Flipkart_URL", "")
        fk_cell = f'<a href="{fk_url}" target="_blank" rel="noopener">{fk_url}</a>' if pd.notna(fk_url) and fk_url else '<span style="color:#bbb;">Not Listed</span>'
        product_rows += (f"<tr style='{row_style}'><td style='{td()}'>{str(r['Title'])[:55]}</td>"
                          ...
                          f"<td style='{td()}'>{r['buy_box_price']}</td><td style='{td()}'>{status_label}</td>"
                          f"<td style='{td()}'>{fk_cell}</td></tr>")
  ...
  <table><tr><th style='{th()}'>Title</th><th style='{th()}'>ASIN</th><th style='{th()}'>Brand</th>
    <th style='{th()}'>Category</th><th style='{th()}'>Your price</th><th style='{th()}'>Buy box price</th><th style='{th()}'>Status</th>
    <th style='{th()}'>Flipkart</th></tr>
    {product_rows}
  </table>
...
def render_top50_tab(top50_df):
  ...
  <table><tr><th style='{th()}'>Title</th><th style='{th()}'>ASIN</th><th style='{th()}'>Brand</th>
    <th style='{th()}'>Type</th><th style='{th()}'>Your price</th><th style='{th()}'>Buy box price</th><th style='{th()}'>Status</th>
    <th style='{th()}'>Flipkart</th></tr>
    {product_rows}
  </table>
...
def render_overview_tab(all_df, fitment_df, od_df, latched_df, top50_df):
    ...
    fk_matched = int(all_df["Flipkart_URL"].notna().sum())
    fk_total = len(all_df)
    fk_pct = round(100 * fk_matched / fk_total, 1) if fk_total else 0.0

    def flipkart_card():
        return f"""<div class="card" style="cursor:default;">
            <div class="card-value" style="color:#2F5496;">{fk_matched}/{fk_total} ({fk_pct}%)</div>
            <div class="card-label">Matched to a Flipkart listing</div>
            <div class="card-sub">{fk_total - fk_matched} listings have no FK counterpart mapped yet</div></div>"""
...
def render_trends_tab(history_df):
    ...
    top500_pct = history_df.get("top500_win_pct", pd.Series([None]*len(history_df))).tolist()
    ...
    for i in range(len(history_df)):
        t500 = "n/a" if pd.isna(top500_pct[i]) else f"{top500_pct[i]}%"
        table_rows += (f"<tr><td style='{td()}'>{dates[i]}</td><td style='{td()}'>{totals[i]}</td>"
                        f"<td style='{td()}'>{t500}</td><td style='{td()}'>{top50_pct[i]}%</td>"
                        f"<td style='{td()}'>{latched_pct[i]}%</td></tr>")

    return f"""
  <h2>Day-on-day trend</h2>
  <p class="hint" style="font-size:12px; color:#999; margin-bottom:10px;">
    Top 500 and Top 50 are our own priority ASIN lists; Latched On is every listing with active
    competition (the genuinely contested Buy Box fights) - kept separate from Only Delivery /
    Fitment listings, which have little or no real competition.</p>
  <div style="position:relative; width:100%; height:280px; margin-bottom:20px;">
    <canvas id="trendChart"></canvas>
  </div>
  <table>
    <tr><th style='{th()}'>Date</th><th style='{th()}'>Total listings</th><th style='{th()}'>Top 500 win %</th>
      <th style='{th()}'>Top 50 win %</th><th style='{th()}'>Latched On win %</th></tr>
    {table_rows}
  </table>
  ...
  <script>
    new Chart(document.getElementById('trendChart'), {{
      ...
      data: {{
        ...
        datasets: [
          {{ label: 'Top 500', data: {json.dumps(top500_pct)}, borderColor: '#2a78d6', backgroundColor: 'rgba(42,120,214,0.08)', borderWidth: 2, pointRadius: 3, tension: 0.2, fill: true, spanGaps: true }},
          {{ label: 'Top 50', data: {json.dumps(top50_pct)}, borderColor: '#eb6834', borderWidth: 2, pointRadius: 3, tension: 0.2, spanGaps: true }},
          {{ label: 'Latched On', data: {json.dumps(latched_pct)}, borderColor: '#2ca02c', borderWidth: 2, pointRadius: 3, tension: 0.2, spanGaps: true }}
        ]
      }},
...
def render_dashboard(all_df, fitment_df, od_df, latched_df, top50_df, history_df, top500_content):
...
<body>
  ...
  <div class="tabs">
    ...
    <button class="tab-button" onclick="switchTab('top500')">Amazon Top 500</button>
  ...
  <div id="tab-top500" class="tab-content">{top500_content}</div>
...
function showDrilldown(tabId, key, label) {{
  ...
  if (rows.length === 0) {{
    ...
    let html = '<table><tr><th>Title</th><th>ASIN</th><th>Brand</th><th>Category</th><th>Your price</th><th>Buy box price</th><th>Gap (losing)</th><th>Headroom (winning)</th><th>Flipkart</th></tr>';
    rows.forEach(r => {{
      const asinCell = r.Product_URL ? '<a href="' + r.Product_URL + '" target="_blank" rel="noopener">' + r.ASIN + '</a>' : r.ASIN;
      const fkCell = r.Flipkart_URL ? '<a href="' + r.Flipkart_URL + '" target="_blank" rel="noopener">' + r.Flipkart_URL + '</a>' : '<span style="color:#bbb;">Not Listed</span>';
      html += '<tr><td>' + String(r.Title || '').slice(0, 55) + '</td><td>' + asinCell +
              ...
              '</td><td>' + (r.gap_to_buybox !== undefined && r.gap_to_buybox !== '' ? r.gap_to_buybox : '—') + '</td>' +
              '<td>' + (r.headroom !== undefined && r.headroom !== '' ? ('+₹' + r.headroom) : '—') + '</td>' +
              '<td>' + fkCell + '</td></tr>';
    }});
    html += '</table>';
    body.innerHTML = html;
...
def main():
    ...
    fk_lookup = build_flipkart_lookup()
    df["base_sku"] = df["SKU"].apply(strip_amazon_suffix)
    df["Flipkart_URL"] = df.apply(
        lambda r: fk_lookup.get((r["base_sku"], r["Listing_Type"])), axis=1
    )
    ...
    amz_top500_df = load_amazon_top500_df()
    if amz_top500_df is not None:
        top500_asins = set(amz_top500_df["asin"])
        top500_win_df = df[df["ASIN"].isin(top500_asins)].copy()
    else:
        top500_win_df = df.iloc[0:0].copy()  # empty, same columns - master file not available yet
    ...
    history_row = pd.DataFrame([{
        ...
        "top500_win_pct": compute_metrics(top500_win_df)["buy_box_win_pct"] if len(top500_win_df) else None,
    }])
    if os.path.isfile(HISTORY_FILE):
        try:
            existing = pd.read_csv(HISTORY_FILE)
            # Align columns rather than blind-append, so adding a new tracked metric
            # (like top500_win_pct here) doesn't silently misalign older rows.
            combined = pd.concat([existing, history_row], ignore_index=True, sort=False)
        except Exception:
    ...
    html = render_dashboard(df, fitment_df, od_df, latched_df, top50_df, history_df, build_amazon_top500_tab(df, top500_win_df))
