...
def build_amazon_top500_tab(df):
    """Market-wide opportunity view mirroring the catalogue dashboard's
    'Amazon Top 500' tab: are we live on the category's actual best-selling
    ASINs (including competitor products), regardless of who else sells them?"""
    if not os.path.isfile(MASTER_FILE):
    ...
    try:
        ...
        return f"<p class='note'>Could not read the 'Amazon Top 500' sheet from the master catalogue file: {e}</p>"
  ...
  <p class="hint" style="font-size:12px; color:#999; margin-bottom:20px;">
    ...
    Measures whether TNM has ANY live listing on the market's actual top sellers - separate from
    Buy Box win rate on our own listings.</p>
...
def render_product_tab(tab_id, df, priority_title, flag_if_under_100=False):
    ...
    for _, r in product_rows_df.iterrows():
        ...
        fk_cell = f'<a href="{fk_url}" target="_blank" rel="noopener">View</a>' if fk_url else '<span style="color:#bbb;">-</span>'
...
function showDrilldown(tabId, key, label) {{
  ...
  if (rows.length === 0) {{
    ...
    rows.forEach(r => {{
      ...
      const fkCell = r.Flipkart_URL ? '<a href="' + r.Flipkart_URL + '" target="_blank" rel="noopener">View</a>' : '<span style="color:#bbb;">-</span>';
...
def main():
    ...
    html = render_dashboard(df, fitment_df, od_df, latched_df, top50_df, history_df, build_amazon_top500_tab(df))
