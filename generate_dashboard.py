...
def build_drilldowns(df):
    ...
    def rows(sub_df):
        cols = ["ASIN", "Title", "Brand", "Category", "Your_Price", "buy_box_price",
                "gap_to_buybox", "num_offers", "Product_URL", "Flipkart_URL"]
...
function showDrilldown(tabId, key, label) {{
  ...
  if (rows.length === 0) {{
    ...
    let html = '<table><tr><th>Title</th><th>ASIN</th><th>Brand</th><th>Category</th><th>Your price</th><th>Buy box price</th><th>Gap</th><th>Flipkart</th></tr>';
    rows.forEach(r => {{
      ...
      html += '<tr><td>' + String(r.Title || '').slice(0, 55) + '</td><td>' + asinCell +
              ...
              '</td><td>' + (r.gap_to_buybox !== undefined ? r.gap_to_buybox : '') + '</td>' +
              '<td>' + fkCell + '</td></tr>';
    }});
    html += '</table>';
    body.innerHTML = html;
